 ...
 *
 * GitHub returns a per-plan host in the token exchange response (individual plans
 * use `https://api.individual.githubcopilot.com`). Falling back to the generic
 * host works for some accounts but not all, so prefer the discovered value.
 *
 * @returns Base URL with no trailing slash
 */
export function getCopilotApiBase(): string {
  const discovered = copilotToken?.endpoints?.api;
  const base =
    config.github.copilot.apiBaseOverride ||
    discovered ||
    config.github.copilot.anthropicEndpoints.COPILOT_API_BASE;

  return base.replace(/\/+$/, '');
}

/**
 * Ensure a valid Copilot token is available, refreshing it if it has expired.
 *
 * @returns The valid token, or null if authentication is not possible
 */
export async function ensureValidCopilotToken(): Promise<CopilotToken | null> {
  if (isTokenValid()) {
    return copilotToken;
  }
  ...
  try {
    logger.info('Copilot token expired or missing, refreshing on demand');
    return await refreshCopilotToken();
  } catch (error) {
    logger.error('On-demand Copilot token refresh failed:', error);
    return null;
  }
}
